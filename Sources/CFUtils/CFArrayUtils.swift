...
/// Errors for the extensions for CFArray.
...
extension CFArray {
	...
	public func convertToArray<T>(of type: T.Type) throws -> [T]  {
		let count = self.count
		var items = [T]()
		for i in 0..<count {
	...
	public func item<T>(at index:Int, of type: T.Type) throws -> T {
	...
	public func map<T>(transform: (UnsafeRawPointer)->T) throws -> [T] {
		let count = self.count
		var items = [T]()
		for i in 0..<count {
